...
from workers.browser.providers.dry_run import DryRunFlowProvider
...
class FlowProvider(BrowserProvider):
    def __init__(
        self,
        ...
        *,
        ...
        if settings.browser_provider_mode == "dry_run":
            self._provider: BrowserProvider = DryRunFlowProvider(settings.playwright_download_root)
        else:
            if selector_bundle is None or session_descriptor is None:
                raise ValueError("Playwright Flow provider requires selector/session data.")
            self._provider = _PlaywrightFlowProvider(
                settings,
...
class _PlaywrightFlowProvider(BrowserProvider):
    def __init__(
        self,
        settings: BrowserWorkerSettings,
        *,
    ...
    def ensure_session(self) -> None:
        ...
        self._session = launch_provider_session(
            ...
            start_url=self._settings.flow_workspace_url,
        )
    ...
    def open_workspace(self) -> None:
        ...
        if self._has_selector("auth_gate"):
            raise BrowserManualInterventionRequired(
                "Flow login or verification is required before visual generation can continue.",
                category="authentication",
            )
        self._resolve("workspace_shell")

    def submit_job(self, payload: ProviderJobPayload) -> str:
        self._fill("scene_prompt_input", payload.prompt or "")
        self._click("batch_submit_button")
        job_id = f"flow-{uuid4()}"
    ...
    def wait_for_completion(self, job_id: str) -> None:
        del job_id
        self._resolve("render_ready_badge")
    ...
    def _workspace_url(self) -> str:
        url = (self._settings.flow_workspace_url or "").strip()
        if not url:
            raise ValueError("FLOW_WORKSPACE_URL must be configured for playwright mode.")
        return url
